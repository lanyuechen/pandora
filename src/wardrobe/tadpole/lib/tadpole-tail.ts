...
export default class TadpoleTail {
	tadpole: Tadpole;
	joints: JointType[];
	animationRate: number;

	constructor(tadpole: Tadpole) {
	...
	update() {
		this.animationRate += (0.2 + this.tadpole.momentum / 10);
	...
	draw(ctx: any) {
		...
		for(var i = 0, len = this.joints.length; i < len; i++) {
			...
			path0.push({x: x1, y: y1});
			path1.push({x: x2, y: y2});
		}

		for(var i = 0; i < path0.length; i++) {
			ctx.lineTo(path0[i].x, path0[i].y);
		}
		path1.reverse();
